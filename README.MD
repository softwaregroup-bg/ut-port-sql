...
module.exports = function sql() {
    return {
        ...
        schema: [{
            path: path.join(__dirname, 'schema1'), linkSP: true
        }, {
...
database schema. In addition, procedures from the first folder will be automatically
linked to the port, so that they can be executed directly, based on their schema
name and procedure name. So for the following procedure:
...
- `bus.importMethod('module.entity.action[^]')(params)` - will resolve with
...
- `bus.importMethod('module.entity.action[0]')(params)` - will resolve with
...
- `bus.importMethod('module.entity.action[+]')(params)` - will array holding
...
- `bus.importMethod('module.entity.action[]')(params)` - will resolve with array
...
Calling `bus.importMethod('module.entity.action')(params)` will resolve with
`{result1:[], result2:[]}`
...
- Setting the `ngram.index` to true will create a table named
`[schemaName].[tableNameIndex]` for storing the n-grams
...
corresponding index table. If the procedure modified more than one row, it
should use. For example if a variable named `@inserted` contains the rows inserted
in the main table, the following code could be used to save the n-grams in the index:
